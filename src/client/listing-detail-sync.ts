import { buildListingDisplay, type ListingDisplayInput } from '../lib/listings/listing-display';
...
function renderFactGroups(groups: ReturnType<typeof buildListingDisplay>['factGroups']): string {
...
function renderCostSummary(display: ReturnType<typeof buildListingDisplay>): string {
...
function renderContact(display: ReturnType<typeof buildListingDisplay>): string {
...
function renderCompareRow(row: ListingPageSurfaceCompareRow, listingIdValue: string): string {
...
function renderTravel(surface: ListingPageSurface, listingIdValue: string): string {
  ...
  const rows = [
    ...surface.travel.compareRows.map((row) => renderCompareRow(row, listingIdValue)),
    ...surface.travel.listingPlaces.map((place) => renderListingPlaceRow(place)),
  ].join('');
...
function renderTourStatus(tour: ListingPageSurface['tour'], basePath: string): string {
  const assignment = tour.assignment;
  if (!assignment) {
    return '<span class="muted">Not on a tour</span>';
  }
  const timeSuffix = assignment.formattedTime ? ` · ${escapeHtml(assignment.formattedTime)}` : '';
...
function renderTourAssignment(tour: ListingPageSurface['tour'], basePath: string): string {
  const assignment = tour.assignment;
...
export function applyListingSurface(listing: ListingDisplayInput): void {
  ...
  if (contactHost instanceof HTMLElement) {
    ...
    contactHost.hidden = !display.address && !display.sourceUrl && !display.phone;
  }
  ...
  if (emptyHost instanceof HTMLElement) {
    emptyHost.hidden =
      display.hasFactSections || display.monthlyTotal != null || display.moveInTotal != null;
  }
...
export function applyListingPageSurface(surface: ListingPageSurface): void {
  ...
  if (tourPanel instanceof HTMLElement) {
    const existingAssignment = tourPanel.querySelector('[data-listing-tour-assignment]');
  ...
  if (travelHost instanceof HTMLElement && id) {
    ...
    document.dispatchEvent(new CustomEvent('wayhome:listing-travel-hydrate', { bubbles: true }));
  }
  ...
  if (mapEl instanceof HTMLElement) {
    ...
    document.dispatchEvent(new CustomEvent('wayhome:listing-map-refresh', { bubbles: true }));
  }
  ...
  document.dispatchEvent(
    new CustomEvent('wayhome:listing-surface-applied', { detail: { surface }, bubbles: true }),
  );
}
...
export async function refreshListingPageSurface(options?: {
  ...
  const url = new URL(`/api/listings/${encodeURIComponent(id)}/surface`, window.location.origin);
...
export function bootListingDetailSync(): void {
  ...
  document.addEventListener('wayhome:listing-updated', (e) => {
    const listing = (e as CustomEvent<{ listing?: ListingDisplayInput }>).detail?.listing;
  ...
  document.addEventListener('wayhome:listing-refresh-surface', (e) => {
    const detail = (e as CustomEvent<{ tourDay?: string; closePlaceOverlay?: boolean }>).detail;
